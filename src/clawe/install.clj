...
(defn command-bin-path [cmd]
  (str (r.config/local-bin-dir) "/" "clawe-" (:name cmd)))

(defn write-temp-main-ns [cmd]
  (r.notify/notify "Writing temp ns" (:name cmd))
  (spit (temp-ns-path cmd)
        (str "(ns clawe.temp (:require ["
             ...
             " nil {:arguments args}))"))
  (r.notify/notify "Wrote temp ns" (:name cmd)))

(comment
  (apply str (next (str :some.name/space))))

(defn create-temp-uberscript [cmd]
  (r.notify/notify "Creating temp uberscript" (:name cmd))
  (-> ^{:dir (project-dir)}
      ...
      :out
      slurp)
  (r.notify/notify "Created temp uberscript" (:name cmd)))

(defn carve-temp-uberscript [cmd]
  (r.notify/notify "Carving temp uberscript" (:name cmd))
  ...
  ;;     check :out slurp)
  (r.notify/notify "Carved temp uberscript" (:name cmd)))
...
exec bb " (temp-uberscript-path cmd) " $@"))
  ...
  (r.notify/notify "Created wrapper script" (:name cmd)))
...
(defn install-micro-handler
  ...
  ([config {:keys [arguments]}]
   (let [cmd (some-> arguments
     ...
     (if cmd
       (do
         (r.notify/notify (str "Installing micro handler for: " (:name cmd)) cmd)
         (try
       ...
       (r.notify/notify (str "No command selected for installation"))))))

(defcom install-micro-cmd
  {:defcom/name    "install-micro"
