...
(fn spawn-fn [cmd] (fn [] (pp cmd) (if (. spawn-fn-cache cmd) (do (naughty.notify {:title "Dropping binding call" :text cmd}) (gears.timer {:timeout 5 :callback (fn [] (tset spawn-fn-cache cmd nil))})) (do (tset spawn-fn-cache cmd true) (awful.spawn.easy_async cmd (fn [stdout stderr _exitreason _exitcode] (tset spawn-fn-cache cmd nil) (when stdout (print stdout)) (when stderr (print stderr))))))))
;; global bindings
